...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      const url = "/analysis/getHistoryData"
      this.$post(url, {
        'isCollect': 1,
        keyWordName: "中海物业"
    ...
    upLoad() {
      ...
      this.$post(url, {
        ...
        require.ensure([], () => {
          ...
          this.export_json_to_excel(tHeader, data, '舆情监测')
        })
      })
    },
    ...
    serachAll() {
      ...
      //   		this.emotion = this.emotion === "全部" ? "" : this.emotion
      // const url="analysis/getHistorySearchData";
      // if(this.mediaType == ''&&this.emotion == ''&&this.filterDate == ''&&city == []){
      // //				this.getData();
      // }else{
      // 	var data1 = {
      ...
      // 	}
      // 	this.$post(url,data1,this.yqURl).then((res) => {
      // 		if(res.data.count == 0){
      // 			this.isShow = true;
      // 			this.allNUm = 0;
      // 		}else{
    ...
    handleCurrentChange(val) {
      // console.log(`当前页: ${val}`);
      // this.mediaType = "全部"? '':this.mediaType;
      // this.emotion = "全部"? '':this.emotion;
      ...
      //	            if (data[prop]===''|| data[prop].length === 0||data[prop] == null) {
      //	                delete data[prop]
      //	            }
      //	        }
      //console.log(data)
