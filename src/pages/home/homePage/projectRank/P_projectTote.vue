<template>
	<div>
		...
		<div class="div_content">
				<ul class="content_ul">
					<li><p>住宅户数(户)</p><span>{{data1}}</span></li>
					<li><p>停车总位数(个)</p><span>{{data2}}</span></li>
					<li><p>车辆入场流量(次)</p><span>{{data3}}</span></li>
					<li><p>剩余车位(个)</p><span>{{data4}}</span></li>
				</ul>
				<ul class="content_ul">
					<li><p>当月总收入(元)</p><span>{{data5}}</span></li>
					<li><p>当月总支出(元)</p><span>{{data6}}</span></li>
					<li><p>当月总增值(元)</p><span>{{data7}}</span></li>
					<li><p>当月增值回报率</p><span>{{data8}}%</span></li>
				</ul>
...
export default {
	...
	methods: {
		...
		getData(){
		 	this.$http({
				url:"http://xhmind.com:5051/tmp/total/project",
				method:'get',
			}).then(function(res){
				if(res.body.status){
					var value_Data =res.body.content;
					this.data1 = value_Data['住宅户数（户）'];
					this.data2 = value_Data['停车位总数（个）'];
					this.data3 = value_Data['车辆入场流量（次）'];
					this.data4 = value_Data['剩余车位（个）'];
					this.data5 = value_Data['当月总收入（元）'];
					this.data6 = value_Data['当月总支出（元）'];
					this.data7 = value_Data['当月总增值（万元）'];
					this.data8 = (value_Data['当月增值回报率']*100);
				}				
			    if(this.isFirst){
			    	//第一次获取数据进行实列化
