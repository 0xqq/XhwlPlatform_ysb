<template>
	<div class="Odiv">
		...
		<p class="title">{{this.yearNow}}年{{this.monthNow}}<i :class="{hide:month==false}">月</i>中海物业集团工程数据分析</p>
		<!--数据显示-->
		<div class="data_div">
...
import Mapvue from '@/components/engineeringMap'
...
export default {
  data () {
	    return {
	    	...
	    	yearNow:'2017',
	    	monthNow:'9',
	    	title:['巡检计划完成率','维修完成率','单位能耗','设备报事数','设备报事处理率'],
			data1:'',
			data2:'',
			data3:'',
			data4:'',
			...
			title5:'设备报事数前十',
			title6:'设备报事处理率前十 ',
			route:'engineeringData',   //向子组件传跳转路由
	    }
  	},
  	components: {
	    Mapvue,
  ...
  methods: {
    ...
    getData(){
		this.loading(true);
	 	this.$http({
			url:this.mnUrl+"/tmp/engineer/overview",
			method:'get',
		}).then(function(res){
			if(res.body.status){
	...
	drawLine() {
		...
		myChart1.on('click', function (params) {
		 	...
		 	refThis.$router.push({path:"C_engineeringData"});			 	
                });
