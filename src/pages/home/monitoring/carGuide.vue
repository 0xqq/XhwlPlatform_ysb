...
export default {
	data() {
		return {
			...
			doors: [{ 'doorName': '1号停车场入口', 'ID': '1' },
			{ 'doorName': '1号停车场出口', 'ID': '2' },
			{ 'doorName': '2号停车场入口', 'ID': '3' },
			{ 'doorName': '2号停车场出口', 'ID': '4' },
			{ 'doorName': '3号停车场入口', 'ID': '5' },
			{ 'doorName': '3号停车场出口', 'ID': '6' }],
			doorName: "出口",
			choisDoor: '',
			options: [{
				value: '选项1',
	...
	methods: {
		getData(){
			const url = this.mnUrl+"/tmp/watching/car/command"
				this.$http.get(url).then(res => {
					for(var i=0;i<res.body.content.length;i++){
					 	//console.log(res)
						this.carList = res.body.content;
						this.sum1.push(res.body.content[i].时间.substring(0,10))
						this.sum2.push(res.body.content[i].时间.substring(11,19))
						var aa=this.sum1[i];
						var bb=this.sum2[i];
						this.all.push(aa+"-"+bb)
					}
			...
			this.$http.get(this.mnUrl+ '/tmp/watching/car/photo').then(res => {
					this.carNums = res.body;
