...
import sourceObj from '@/httpData/netreSource'
import {
  mapState
} from 'vuex'
export default {
  ...
  methods: {
    ...
    getData() {
			var value9 = sourceObj.content
			value9.forEach((item, index) => {
				this.newarr.push(item.时间)
				this.time.push(item.时间.substring(12, 19))
				this.valueList.push(item["网络流入流速（KB/s）"])
				this.valueList2.push(item["网络流出流速（KB/s）"])
			})
			 this.drawLine()
    }
  }
