	 ...
	 <div class="modal">
	     <div class="modal-content">
	        <div class="table_title">
	        	<span>联网项目详情</span>
	            <button class="modal-close" @click="$emit('fireclose')">x</button>
	        </div>
	        <div class="table">
	        	<ul class="table_top">
	        		<li v-for="title in titleData" class="top_li">{{title}}</li>
	        	</ul>
...
export default {
  ...
  methods: {
	getData(){
		this.$http({
			url:this.mnUrl+"/tmp/total/project_number",
			method:'get',
		}).then(function(res){
			if(res.body.status){
...
.table_top{
	...
	display: flex;
	border-bottom: 0.01rem solid #447399;
	.top_li{
		flex: 1;
		color: #fff;
		height: 0.45rem;
		text-align: center;
		line-height:0.45rem;
		border-left: 0.01rem solid #447399;
	}
}
.table_top li:nth-of-type(1){
	border-left: 0;
}
.table_body{
	width: 100%;
...
.content_ul{
	...
	li{
		...
		text-align: center;
		line-height:0.45rem;
		border-left: 0.015rem solid #447399;
		border-bottom: 0.015rem solid #447399;
		font-size: 0.14rem;
	}
}
