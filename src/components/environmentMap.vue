...
export default {
	data() {
			...
			year:function(){
				for(var i=0;i<this.content.length;i++){
			    	...
			    	if(timeNow==this.year){			    		
			    		var num1 = this.content[i].环境工作完成率前十;
						var num2 = this.content[i].生活水质指数前十;
						var num3 = this.content[i].泳池水质指数前十;
						var num4 = this.content[i].景观水质指数前十;
						var num5 = this.content[i].空气质量指数前十;
						for(var i=0;i<num1.length;i++){
							...
							this.data4[i]={name:num2[i].城市,value:num4[i].数值};
							this.data5[i]={name:num3[i].城市,value:num5[i].数值};
						}			    		
			    	}				
				};
				this.drawLine();
		...
		methods: {
			getData(){
		 	this.$http({
				url:this.mnUrl+"/tmp/environment/overview",
				method:'get',
			}).then(function(res){
				...
				for(var i=0;i<value.length;i++){
			    	...
			    	var timeNow =  year+'-'+month;
			    	if(timeNow==this.year){			    		
			    		var num1 = value[i].环境工作完成率前十;
						var num2 = value[i].生活水质指数前十;
						var num3 = value[i].泳池水质指数前十;
						var num4 = value[i].景观水质指数前十;
						var num5 = value[i].空气质量指数前十;
						for(var i=0;i<num1.length;i++){
							this.data1[i]={name:num1[i].城市,value:num1[i].数值};
							this.data2[i]={name:num2[i].城市,value:num2[i].数值};
							this.data3[i]={name:num3[i].城市,value:num3[i].数值};
			...
			var	option = {
				    ...
				    legend: {          //标题组件参数配置（左边，大小，颜色等）
				        ...
				        x:'80',
				        icon: 'rect',
				        data:['环境工作完成率','生活水质指数','泳池水质指数','景观水质指数','空气质量指数'],
				        textStyle: {
				            color: '#fff'
				        },
