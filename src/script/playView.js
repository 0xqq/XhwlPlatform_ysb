...
function initTree(){
	...
	var orderdCellIds = initControlUnitNodes(xmlStr);	//把xml赋给dom树，产生中海物业集团和空ul
	//初始化区域节点
	if(orderdCellIds.length > 0){
		xmlStr = OCXobj.GetResourceInfo(2);		//获取所有
		console.log(xmlStr);	
		var regionIds = initRegionNodes(xmlStr, orderdCellIds);		//把xml赋给dom树，产生广州云麓公馆和空ul
		//初始化监控点节点
